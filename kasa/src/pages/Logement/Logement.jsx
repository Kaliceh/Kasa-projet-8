...
function Logement() {
    ...
    useEffect(() => {
        const fetchLogement = async () => {
            try {
                ...
                if (!logementFind) {
                    setNotFound(true); // déclenche l'affichage du composant Error
                } else {
                    setLogement(logementFind);
                }
            } catch {
                setNotFound(true); // en cas d'erreur fetch
            } finally {
                setLoading(false);
            }
        };
    ...
    if (notFound) return <Error />; // ici on affiche juste le contenu Error
